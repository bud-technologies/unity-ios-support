...
enum REMOTE_INVITATION_STATE {
  ...
  /**
   2: <b>RETURNED TO THE CALLEE.</b> The message is sent to the caller that the call invitation is
   accepted.
   */
...
enum LOCAL_INVITATION_ERR_CODE {
  ...
  /**
   ...
   - Returns this error code, if the callee is still offline 30 seconds since the call invitation is
   sent.
   */
  LOCAL_INVITATION_ERR_PEER_OFFLINE = 1,

  /**
   2: <b>RETURNED TO THE CALLER.</b> The callee is online but has not ACKed to the call invitation
   30 seconds since it is sent.
   */
  LOCAL_INVITATION_ERR_PEER_NO_RESPONSE = 2,

  /**
   3: <b>RETURNED TO THE CALLER. SAVED FOR FUTURE USE.</b> The call invitation expires 60 seconds
   since it is sent, if the callee ACKs to the call invitation but neither the caller or callee
   takes any further action (cancel, accpet, or decline it).
   */
...
enum REMOTE_INVITATION_ERR_CODE {
  ...
  /**
   2: <b>RETURNED TO THE CALLEE.</b> The call invitation received by callee fails: The callee does
   not ACK within a set time after the callee accepts the call invitation. This is usually a result
   of a network interruption.
   */
  REMOTE_INVITATION_ERR_ACCEPT_FAILURE = 2,

  /**
   3: <b>RETURNED TO THE CALLEE.</b> The call invitation expires 60 seconds since it is sent, if the
   callee ACKs to the call invitation but neither the caller or callee takes any further action
   (cancel, accpet, or decline it).
   */
...
enum INVITATION_API_CALL_ERR_CODE {
  ...
  /**
   1: The method call fails. The argument is invalid, for example, the value of the @p content
   parameter exceeds 8K bytes.
   */
...
class ILocalCallInvitation {
 ...
 public:
  ...
  /**
   ...
   @param content The content of the call invitation. The @p content must not exceed 8 KB in length
   if encoded in UTF-8.
   */
  ...
  /**
   ...
   @note The caller sets the call invitation content using the \ref
   agora::rtm::ILocalCallInvitation::setContent "setContent" method.
   */
  ...
  /**
   ...
   @note To intercommunicate with the legacy Agora Signaling SDK, you MUST set the channel ID.
   However, even if the callee successfully accepts the call invitation, the Agora RTM SDK does not
   join the channel of the specified channel ID.
  ...
  /**
   ...
   @note The callee sets his/her response using the \ref
   agora::rtm::IRemoteCallInvitation::setResponse "setResponse" method.
   */
...
class IRemoteCallInvitation {
 ...
 public:
  ...
  /**
   ...
   @param response The callee's response to the call invitation. The @p response must not exceed 8
  ...
  /**
   ...
   @note The callee sets a response to the incoming call invitation using the \ref
   agora::rtm::IRemoteCallInvitation::setResponse "setResponse" method.
   */
  virtual const char* getResponse() const = 0;
...
class IRtmCallEventHandler {
 public:
  ...
  /**
   Callback to the caller: occurs when the life cycle of the outgoing call invitation ends in
   failure.
  ...
  /**
   ...
   @param response The callee's response to the call invitation.
       */
  virtual void onLocalInvitationAccepted(ILocalCallInvitation* localInvitation,
                                         const char* response) {
    (ILocalCallInvitation*)localInvitation;
    (const char*)response;
  }
  ...
  /**
   ...
   @param response The callee's response to the call invitation.
   */
  virtual void onLocalInvitationRefused(ILocalCallInvitation* localInvitation,
                                        const char* response) {
    (ILocalCallInvitation*)localInvitation;
    (const char*)response;
  }
  ...
  /**
   Callback to the callee: occurs when the life cycle of the incoming call invitation ends in
   failure.
